...
fileInput.addEventListener("change", () => {
  ...
  // 🔥 VALIDASI 2: CHECK FILE SIZE (5 MB MAX)
  ...
  if (file.size > MAX_FILE_SIZE) {
    ...
    dropPreviewImg.classList.add("hidden");
...
detectBtn.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (rawStatus.includes("yakin")) {
      explanationText = data.description || "";

    } else if (rawStatus.includes("perkiraan")) {
      explanationText =
        (data.message || "") + "\n\n" +
      ...
      rawStatus.includes("tidak") ||
      rawStatus.includes("low")
    ) {
      explanationText =
      ...
      explanationText = "Tidak ada deskripsi tersedia.";
    }
